...
- **一句话描述**：面向遗传科/儿科/内科执业医师的罕见病诊断辅助系统，以**循证医学（EBM）为核心**，将临床诊断推理过程形式化为五层递进分析，每一步均附多源文献证据支撑。
- **解决的痛点**：
  - 罕见病知识分散，医生难以快速整合多源循证证据
  - 表型-疾病映射复杂，依赖医生记忆和经验
  - 诊断过程缺乏结构化记录和可审计性
  - 鉴别诊断推理链不透明，难以复现和教学
  - 缺乏标准化的证据分级和文献追溯机制
  - 跨学科会诊时难以共享统一的诊断推理过程
- **目标受众**：罕见病诊疗中心执业医师、医学研究员、遗传咨询师、临床药师及多学科诊疗（MDT）团队

---

## 2. 功能特性

- **循证医学核心**：每层推理均自动检索 KnowS 多源医学证据（PubMed 英文/中文、指南、临床试验、药品说明书），输出结构化证据池，支持证据分级（A-E）和 DOI 追溯
- **五层推理引擎**：模拟临床鉴别诊断的完整思维链（表型分析 → 贝叶斯假设 → 时序推理 → 遗传推理 → 诊断路径规划），每层均有循证证据支撑
- **全量表型词典**：11,606 个 HPO 术语（频率表加载）+ LLM 语义匹配自动学习缓存
- **贝叶斯概率推理**：基于 Orphanet/OMIM 疾病-表型频率表的量化诊断，支持人口学先验修正，所有假设均附证据强度评分
- **时序推理**：发病年龄、进展速度、症状序列作为诊断信号
- **遗传推理**：家系分析 + 孟德尔遗传模式推断 + 回流验证
- **诊断路径规划**：基于 EVOI（信息期望值）推荐最优下一步检查，每项检查均附方法学证据支撑
- **循证证据集成**：KnowS 6 源医学证据检索（PubMed 英文/中文、指南、临床试验、药品说明书），每层推理均自动检索并分级（A-E），支持 DOI/PMID 追溯原文
- **证据弹窗系统**：每个推理节点右上角 `🅔 N` 徽标 → 点击弹出证据明细（含标题、期刊、DOI、PMID、研究类型、证据等级）→ 有 DOI 的文献可跳转原文
